...
export default function FashionFormStep({ data, onChange, onBack, onSubmit }: Props) {
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Prénom / Nom *" value={data.name} onChange={(v) => onChange({ name: v })} placeholder="Léa Dubois" required />
          <Field label="Titre / Spécialité *" value={data.title} onChange={(v) => onChange({ title: v })} placeholder="Mannequin, Styliste, DA…" required />
        </div>

        <Field label="Email de contact *" type="email" value={data.email} onChange={(v) => onChange({ email: v })} placeholder="lea@exemple.com" required />
        ...
        <Field label="LinkedIn (optionnel)" value={data.linkedinUrl} onChange={(v) => onChange({ linkedinUrl: v })} placeholder="linkedin.com/in/lea" />
