...
export default function FashionFormStep({ data, t, onChange, onBack, onSubmit }: Props) {
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label={t.form.nameLabel} value={data.name} onChange={(v) => onChange({ name: v })} placeholder="Léa Dubois" required />
          <Field label={t.fashion.titleLabel} value={data.title} onChange={(v) => onChange({ title: v })} placeholder="Mannequin, Styliste, DA…" required />
        </div>

        <Field label={t.form.emailLabel} type="email" value={data.email} onChange={(v) => onChange({ email: v })} placeholder="lea@exemple.com" required />
        ...
        <div>
          ...
          <div className="flex items-center rounded-xl transition"
            ...
            <input type="text" value={data.instagramHandle}
              ...
              placeholder="leadubois"
              className="w-full bg-transparent px-3 py-3 text-sm outline-none"
              style={{ color: "#1c1917" }} />
          </div>
        </div>

        <Field label={t.form.linkedinLabel} value={data.linkedinUrl} onChange={(v) => onChange({ linkedinUrl: v })} placeholder="linkedin.com/in/lea" />
